...
<body>
    ...
    <div class="content">
        ...
        <div class="container">
        <br/>
<div class="medicine-title">Low in Stock</div><hr>
<br/>
...
$sql = "SELECT * FROM batch WHERE batch_qty<15 LIMIT $start_from, $records_per_page";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<tr>
            <th class='medicine-table-header'>Batch ID</th>
            <th class='medicine-table-header'>Medicine Name</th>
            <th class='medicine-table-header'>Brand</th>
            <th class='medicine-table-header'>Exp Date</th>
            <th class='medicine-table-header'>Price</th>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td class='medicine-table-cell medicine-id'>" . $row['batch_id'] . "</td>";
            echo "<td class='medicine-table-cell medicine-name'>" . $row_med['medicine_name'] . "</td>";
            echo "<td class='medicine-table-cell'>" . $row_med['medicine_brand'] . "</td>";
            echo "<td class='medicine-table-cell'>" . $row['batch_expdate'] . "</td>";
            echo "<td class='medicine-table-cell'>" . $row['batch_price'] . "</td>";
            echo "<td class='medicine-table-cell'>" . $row['batch_qty'] . "</td>";
            echo "<td class='medicine-table-cell'>";
            if ($row['batch_status'] == 'Available') {
                echo "<label class='switch'><input type='checkbox' checked='checked' data-medicineid='" . $row['batch_id'] . "'><span class='slider round'></span></label>";
            } else {
                echo "<label class='switch'><input type='checkbox' data-medicineid='" . $row['batch_id'] . "'><span class='slider round'></span></label>";
            }
            echo "</td>";
            echo "<td class='medicine-table-cell'><a class='medicine-link' href='updatebatch.php?batch_id=" . $row['batch_id'] . "'>Update</a></td>";
        ...
        echo '<center><img src="' . $image_url . '" alt="No results" width="480" height="400"></center>';
    }

    // Count the total number of records in the medicine table
    $sql_count = "SELECT COUNT(*) as total_records FROM batch";
...
?>
<a href="addbatch.php"><button class="btn-add">Add</button></a>
<script>
    ...
    for (var i = 0; i < switchButtons.length; i++) {
        switchButtons[i].addEventListener('change', function() {
            ...
            var batch_id = this.getAttribute('data-medicineid');

            // Send an AJAX request to update the database
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'deletebatch.php');
            ...
            xhr.send('batch_id=' + batch_id + '&batch_status=' + isChecked + '&page=' + <?php echo $current_page; ?>);
